...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    // コンピューターが動かす車用の変数
    int computerX = WIDTH / 2;
    int computerY = -100;
    int computerType = YELLOW;
    ...
    while (1) // メインループ
    {
        ...
        // コンピューターの車を動かす処理
        computerY = computerY + 4;
        if (computerY > HEIGHT + 100) // 画面の下から外に出たか
        {
            computerX = rand() % 180 + 270;
            computerY = -100;
            computerType = YELLOW + rand() % 3;
        }
        drawCar(computerX, computerY, computerType);
